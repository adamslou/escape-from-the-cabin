...
** Author:       Louis Adams
** Date:         2019-06-03     		             	
** Description:  This class represents a kitchen space in the Escape from McFarland game.
...
* if that item should exist in the Kitchen space at the start of the game.     *
*******************************************************************************/
Kitchen::Kitchen() : Space {KITCHEN}
{   items = {Item(ROPE, "rope", false), 
             Item(BED_POSTS, "wooden bed posts", false),
             Item(NOODLES, "a packet of ramen noodles", true), 
             Item(POT, "a cooking pot", true),
             Item(WATER_JUG, "a water jug", true),
             Item(COOKBOOK, "a wilderness cookbook", true),
             Item(EGGS, "a pair of eggs", true),
...
/*******************************************************************************
*                           Kitchen::~Kitchen Destructor                       *
* This is a virtual destructor for the Kitchen class.                        *
*******************************************************************************/
Kitchen::~Kitchen()
{
}
...
* space. For the kitchen the user can add items, drop items, or move to        *
* another space. The player can move to the living room or shore 2 from the    *
* dock.                                                                        *
*******************************************************************************/
void Kitchen::takeTurn(std::vector<Item> &inventory, int &numInventory, int &turn, Space *&playerLoc, 
...
{   int choice;     // Stores's the user's menu choice
    ...
    {   choice = playerLoc->printMenu(playerLoc);   // Maybe have to do playerLoc->printMenu();
        ...
        {   case 1:
            ...
            case 4:
            {   playerLoc = playerLoc->getBottom();     // Moves player to the shore 2
                break;
            }
            ...
            case 6:
            {   playerLoc = playerLoc->getRight();      // Moves player to the living room
                break;
            }
        }
    }while(choice != 4 && choice != 6);    // Loop until user selects 4 or 6 to move out of the kitchen
}
...
* Kitchen space.                                                               *
*******************************************************************************/
int Kitchen::printMenu(Space *playerLoc)
{   int choice;     // Holds the user's menu choice

    std::cout << "\nLooking around the kitchen you see the following items:\n";
...
*******************************************************************************/
void Kitchen::printMap(Space *playerLoc)
{
    std::cout << '\n';
    std::cout 
        << "   ^   ^                ^                         ^                                 ^   \n"
