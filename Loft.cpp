...
* if that item should exist in the Loft space at the start of the game.        *
*******************************************************************************/
Loft::Loft() : Space {LOFT}
{   items = {Item(ROPE, "rope", true), 
             Item(BED_POSTS, "wooden bed posts", true),
...
/*******************************************************************************
*                           Loft::~Loft Destructor                             *
* This is a virtual destructor for the Loft class.                             *
*******************************************************************************/
Loft::~Loft()
{
}
...
* space. For the loft the user can add items, drop items, or move to           *
* another space. The player can move to the living room from the loft.         *
*******************************************************************************/
void Loft::takeTurn(std::vector<Item> &inventory, int &numInventory, int &turn, Space *&playerLoc, 
...
{   int choice;     // Stores the user's menu choice
    ...
    {   choice = playerLoc->printMenu(playerLoc);   // Maybe have to do playerLoc->printMenu();
        ...
        {   case 1:
            ...
            case 4:
            {   playerLoc = playerLoc->getBottom();     // Moves player to the living room   
                break;
            }
    ...
    }while(choice != 4);    // Loop until user selects 4 to move out of the loft
}
...
* Loft space.                                                                  *
*******************************************************************************/
int Loft::printMenu(Space *playerLoc)
{   int choice;     // Holds the user's menu choice

    std::cout << "\nLooking around the loft you see the following items:\n";
...
*******************************************************************************/
void Loft::printMap(Space *playerLoc)
{
    std::cout << '\n';
    std::cout 
        << "   ^   ^                ^                         ^                                 ^   \n"
